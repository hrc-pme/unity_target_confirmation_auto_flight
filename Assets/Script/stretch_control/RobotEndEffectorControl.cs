...
public class RobotEndEffectorControl : MonoBehaviour
{
    public Transform rollJoint;
    public JointStateWriter rollJointWriter; 
    public Transform pitchJoint;
    public Transform yawJoint;
    public Transform vrJoystickInput; // VR joystick rotation as a Transform
    ...
    void Update()
    {
        if (vrJoystickInput != null && rollJoint != null && pitchJoint != null && yawJoint != null)
        {
            // Get the local rotation of the joystick
            Vector3 joystickLocalEulerAngles = vrJoystickInput.localRotation.eulerAngles;

            // Extract angles and normalize them
            float rotateRoll = NormalizeAngle(joystickLocalEulerAngles.z);
            float rotatePitch = NormalizeAngle(joystickLocalEulerAngles.x);
            float rotateYaw = NormalizeAngle(joystickLocalEulerAngles.y);
